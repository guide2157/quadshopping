...
public class Main {
    ...
    public static void main(String[] args) {
        String value = String.format("%.2f", 4.52135);
    ...
    private static void assignPrice() {
        ...
        double avergePrice = totalPrice / people.size();
        for (Person person : people) {
            double difference = avergePrice-person.getTotalSpent();
    ...
    private static void addItem(){
        ...
        int foundPerson;
        while(true){
            foundPerson = Collections.binarySearch(people,buyer1,null);
            if (foundPerson>=0){
                break;
            } else if(buyer.equals("q")){
                break;
            }
            else{
                System.out.println("Buyer not found in member list. Please enter buyer name again or type q to cancel.");
                buyer=scanner.nextLine().toLowerCase();
                buyer1 = new Person(buyer);
            }
        }
        if(!buyer.equals("q")) {
            Product product = new Product(name, price, day, month, buyer);
    ...
    private static int addMonth(){
        ...
        while(true){
            if ((month>=1) &&(month<=12)){
                ...
                System.out.println("Entered month not in range. Please enter a month number between 1-12 :");
                month=scanner.nextInt();
            }
        }
        return month;
    }
    private static int addDay(int month){
        int day;
        System.out.println("Enter day: ");
        day=scanner.nextInt();
        ...
        while(correct){
            ...
            if(correct) {
                System.out.println("Entered day not in range. Please enter again:");
                day = scanner.nextInt();
            }
